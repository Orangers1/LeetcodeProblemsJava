...
public class ValidSudoku {
     ...
     * assumes that input is a 9x9 character array as problem states, solution is in o(n^2) time, where n is the number of rows in the input
     */
    public boolean isValidSudoku(char[][] board) {
        ...
        for (int i = 0; i < board.length; i++) {
            ...
            for(int j = 0; j < board[i].length; j++) {
                if (seenDigits.contains(board[i][j])) {
                    return false;
                } else{
                    if(board[i][j] != '.') { // ignore empty spaces for this check
                        seenDigits.add(board[i][j]);
                    }
                }
            }
        }
